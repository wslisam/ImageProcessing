...
int find_num_obj_using_ConnectedComponents(cv::Mat img) {
  ...
  cv::Mat labels;
  int num_objects =
      connectedComponents(img, labels);  // connectivity=8 , type = CV_32S
  ...
  for (int y = 0; y < resized_height; y++) {
    ...
    for (int x = 0; x < resized_width; x++) {
      ...
      for (int c = 0; c < channel; c++) {
        val =
            (1.0 - dx) * (1.0 - dy) * img.at<cv::Vec3b>(y_before, x_before)[c] +
...
cv::Mat BGR2GRAY(cv::Mat input) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      out.at<uchar>(y, x) = 0.2126 * (float)input.at<cv::Vec3b>(y, x)[2] +
                            0.7152 * (float)input.at<cv::Vec3b>(y, x)[1] +
                            0.0722 * (float)input.at<cv::Vec3b>(y, x)[0];
    }
  }
...
vector<vector<pair<int, int>>> rect_contours(
    cv::Mat img, vector<vector<cv::Point>> contours) {
...
cv::Mat cal_and_cut(cv::Mat img, cv::Mat mask, int sample_size) {
  ...
  for (int seg = 0; seg < num_of_region; seg++)  // seg = region index
  {
    ...
    cv::Mat m_roi =
        img(cv::Rect(rect_coord[seg][0].first, rect_coord[seg][0].second,
                     rect_coord[seg][1].first - rect_coord[seg][0].first,
                     rect_coord[seg][2].second - rect_coord[seg][0].second));

    cv::Mat mask_roi =
        mask(cv::Rect(rect_coord[seg][0].first, rect_coord[seg][0].second,
                      rect_coord[seg][1].first - rect_coord[seg][0].first,
                      rect_coord[seg][2].second - rect_coord[seg][0].second));
    ...
    rect_roi =
        final(cv::Rect(rect_coord[seg][0].first, rect_coord[seg][0].second,
...
cv::Mat segmentation(cv::Mat img, cv::Mat mask, int *Grid_size_x,
                     int *Grid_size_y, int sample_size) {
  vector<vector<cv::Point>> contours;
  findContours(img, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
  vector<vector<pair<int, int>>> rect_coord = rect_contours(img, contours);
  ...
  for (int seg = 0; seg < num_of_region; seg++)  // seg = region index
  {
    ...
    y_1 = rect_coord[seg][2].second;
    // num_of_sample[seg] = 0;
    *Grid_size_y = (rect_coord[seg][2].second - rect_coord[seg][0].second) /
                   (region_number_y[seg]);
         ...
         y += *Grid_size_y)  // size can the same
    {
      ...
      *Grid_size_x = (rect_coord[seg][1].first - rect_coord[seg][0].first) /
                     (region_number_x[seg]);
...
cv::Mat gen2_segmentation(cv::Mat img, cv::Mat mask, int *Grid_size_x,
                          int *Grid_size_y, int dimension, int *region_num) {
  vector<vector<cv::Point>> contours;
  findContours(img, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
  vector<vector<pair<int, int>>> rect_coord = rect_contours(img, contours);
...
cv::Mat multi_planefit(cv::Mat img, cv::Mat mask_img, int sample_size,
                       int num_of_sample) {
    ...
    for (int current_col = 0; current_col < width; current_col += sample_size) {
      ...
      if (mask_img.at<uchar>(current_row, current_col) > 0) {
        ...
        if (current_col <= (width / 2)) {  // 0134
          ...
          matrix_b.at<float>(count, 0) =
              img.at<uchar>(current_row, current_col);  // y 行 第x個

          matrix_a.at<float>(count, 0) =
              (((width / 2) - current_col) * (height - current_row)) /
              temp;  // (x2-x)(y-y1)
          matrix_a.at<float>(count, 1) =
              ((current_col - 0) * (height - current_row)) /
              temp;  //(x-x1)(y-y1)
          matrix_a.at<float>(count, 3) =
              ((width / 2 - current_col) * (current_row - 0)) /
              temp;  // (x2-x)(y2-y)
          matrix_a.at<float>(count, 4) =
              ((current_col - 0) * (current_row - 0)) / temp;  //(x-x1)(y2-y)

          cout << "val :   "
               << ((width / 2) - current_col) * (height - current_row) / temp
               << endl;  // (x2-x)(y-y1)
          cout << "val :   "
               << (current_col - 0) * (height - current_row) / temp
               << endl;  //(x-x1)(y-y1)
          cout << "val :   "
               << (width / 2 - current_col) * (current_row - 0) / temp
               << endl;  // (x2-x)(y2-y)
          cout << "val :   " << (current_col - 0) * (current_row - 0) / temp
               << endl;  //(x-x1)(y2-y)
          cout << ">>>>>>>>" << endl;
          count++;
          ...
          matrix_a.at<float>(count, 1) =
              ((width - current_col) * (height - current_row)) /
              temp;  // (x2-x)(y-y1)
          matrix_a.at<float>(count, 2) =
              ((current_col - width / 2) * (height - current_row)) /
              temp;  //(x-x1)(y-y1)
          matrix_a.at<float>(count, 4) =
              ((width - current_col) * (current_row - 0)) /
              temp;  // (x2-x)(y2-y)
          matrix_a.at<float>(count, 5) =
              ((current_col - width / 2) * (current_row - 0)) /
              temp;  //(x-x1)(y2-y)
